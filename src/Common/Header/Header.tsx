...
const [targetTop, setTargetTop] = useState<number>(Math.max(-minHeaderHeight / 2 + headerHeight / 2))
...
useEffect(() => {
  if (logoRef.current) {
    const isLarger = headerHeight > minHeaderHeight && headerHeight > currentHeight;
...
// Effect to animate the opacity of the glassEffectContainer based on the state of isMinHeaderHeight.
useEffect(() => {
  gsap.to(
